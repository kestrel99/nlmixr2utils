...
extractEqHelperSeqDrop <- function(x, ..., inModel, dropIdx = 1) {
  ...
  for (idx in setdiff(seq_along(x), dropIdx)) {
    ret <- c(
      ret,
      extractEqHelper(x[[idx]], name = retNames[[idx]], ..., inModel = inModel)
    )
  }
  ret
}
...
extractEqHelperLhsRhs <- function(
  x,
  ...,
  inModel,
  prefix = "",
  ...
  if (inModel) {
    if (is.name(x[[1]])) {
      ...
      if (x1c %in% c("exp", "log")) {
        ...
        ret <- extractEqHelperLhsRhs(
          x,
          ...,
          inModel = inModel,
          middle = latexOpMap[[x1c]]
        )
      } else if (x1c %in% "!") {
        ...
        if (length(x) == 2) {
          ...
          cli::cli_abort(
            "Cannot handle addition or subtraction with length that is not 2 or 3"
          ) # nocov
        }
      } else if (x1c == "*") {
        ret <- extractEqHelperLhsRhs(
      ...
      stop(
        "a call that does not start with a name is in the model, please report a bug"
      ) # nocov
    }
  } else if (length(x) == 1) {
    ...
    if (is.name(x[[1]])) {
      ...
      stop(
        "a call that does not start with a name does not have inModel set, please report a bug"
      ) # nocov
    }
  }
  ret
...
#' @export
extractEqHelper.name <- function(
  x,
  ...,
  inModel,
  underscoreToSubscript = FALSE,
  name
) {
  if (inModel) {
    ret <- as.character(x)
...
braceWrap <- function(x, ..., inModel, indent) {
  if (inherits(x, "{")) {
    innerPart <- extractEqHelperSeqDrop(
      x,
      ...,
      inModel = inModel,
      indent = indent + 1L
...
extractEqHelper.if <- function(
  ...
  firstPart <- sprintf(
    "\\mathrm{if} & \\left(%s\\right) %s",
    conditionalPart,
    bracedPart[1]
  )
  lastPart <-
    if (length(bracedPart) > 1) {
      bracedPart[length(bracedPart)] <- paste(
        bracedPart[length(bracedPart)],
        appendPart[1]
      )
...
extractEqHelper.default <- function(x, ..., inModel) {
  if (inherits(x, "<-") || inherits(x, "=")) {
    ...
    # check issue with replacement function naming requirements.
